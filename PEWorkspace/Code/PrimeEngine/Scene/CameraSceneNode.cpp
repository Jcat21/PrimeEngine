...
void CameraSceneNode::do_CALCULATE_TRANSFORMATIONS(Events::Event *pEvt)
{
	...
	// set frustum equations
	Matrix4x4 wtv = m_viewToProjectedTransform * m_worldToViewTransform;
	//wtv.normalizeUVN();
	for (int i = 0; i < 3; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			m_frustumeqs[i * 2][j] = wtv.m[3][j] + wtv.m[i][j];
			m_frustumeqs[i * 2 + 1][j] = wtv.m[3][j] - wtv.m[i][j];
		}
	}
